...
**Closed 2026-08-30 — the SDK path is the contract.** The SDK `attestIsolation` method (`AIMClient.ts:729`, `client.py:2427`, `AIMClient.java:1698`) POSTs to `/api/v1/sdk-api/agents/<id>/isolation-attestation`. The backend previously registered only `/api/v1/sdk-api/agents/:id/isolation`, so every SDK call 404'd, the `isolation_attestations` table stayed near-empty, and most agents read the `0.3` baseline regardless of posture. Per the recorded architecture decision of 2026-08-29 the SDK path is canonical and is now registered on the same handler; `/api/v1/sdk-api/agents/:id/isolation` remains as a deprecated alias. **Rows only start landing for agents running an SDK build that reaches a backend carrying this fix** — scores for agents talking to an older deployment stay at the baseline. Background and the external-attestation source hierarchy are in [docs/specs/execution-isolation-v1.md](../specs/execution-isolation-v1.md).
