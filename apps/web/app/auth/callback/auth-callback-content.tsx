  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // OAuth error from provider or backend
        const error = searchParams.get("error");
        if (error) {
          const errorMessage = error || "OAuth authorization failed";
          ...
          // Set status and redirect to login
          setStatus("error");
        ...
        throw new Error(
          "Invalid callback: no token or registration request received"
        );
      } catch (err) {
