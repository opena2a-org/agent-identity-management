  ...
  const fetchAuditLogs = async () => {
    try {
      ...
      const filtered = logs.filter((log: AuditLog) => {
        ...
        if (log.action === "view" && log.resource_type === "alerts")
          return false;
        // Exclude view + dashboard_stats (also automated)
        if (log.action === "view" && log.resource_type === "dashboard_stats")
          return false;
        // Exclude view + users (less interesting)
        if (log.action === "view" && log.resource_type === "users")
          return false;
        // Exclude view + audit_logs (less interesting)
        if (log.action === "view" && log.resource_type === "audit_logs")
          return false;
        return true;
      });
