...
from .exceptions import AuthenticationError, TokenExpiredError, InvalidCredentialsError
...
class OAuthTokenManager:
    ...
    def _refresh_token(self) -> Optional[str]:
        ...
        Raises:
            TokenExpiredError: If refresh token has been revoked (expected after rotation)
        ...
        if not self.credentials or 'refresh_token' not in self.credentials:
            raise InvalidCredentialsError(
                ...
                portal_url=self.credentials.get('aim_url', 'http://localhost:3000') if self.credentials else 'http://localhost:3000'
            )

        aim_url = self.credentials.get('aim_url', 'http://localhost:8080')
        portal_url = aim_url.replace(':8080', ':3000')  # Convert API URL to portal URL
        ...
        try:
            ...
            # Handle token rotation/revocation (401 or 403)
            if response.status_code in [401, 403]:
                # Token has been revoked - this is expected after token rotation
                raise TokenExpiredError(portal_url=portal_url)
            ...
            if not self.access_token:
                raise InvalidCredentialsError(
                    reason="Server returned empty access token",
            ...
            if new_refresh_token and new_refresh_token != refresh_token:
                ...
                print("🔄 Token rotated successfully - new credentials saved")
            ...
            if self.access_token:
                try:
                    ...
                    # Non-fatal: we can still use the token
            ...
            # Re-raise our custom exceptions with helpful messages
            raise
        except requests.exceptions.RequestException as e:
            # Network error
            print(f"⚠️  Warning: Network error during token refresh: {e}")
            print(f"    Check that AIM backend is running at: {aim_url}")
            return None
        except Exception as e:
            # Unexpected error
            print(f"⚠️  Warning: Unexpected error during token refresh: {e}")
            return None
