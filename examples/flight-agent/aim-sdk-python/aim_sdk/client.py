...
def _load_credentials(agent_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        agent_creds = all_creds.get(agent_name)
        if not agent_creds:
            return None

        # Merge root-level OAuth tokens with agent credentials
        # This is critical for authentication to work when using existing credentials
        if "refresh_token" in all_creds and "refresh_token" not in agent_creds:
            agent_creds["refresh_token"] = all_creds["refresh_token"]
        if "access_token" in all_creds and "access_token" not in agent_creds:
            agent_creds["access_token"] = all_creds["access_token"]
        if "sdk_token_id" in all_creds and "sdk_token_id" not in agent_creds:
            agent_creds["sdk_token_id"] = all_creds["sdk_token_id"]
